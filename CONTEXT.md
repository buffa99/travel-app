...
- ブラウザが開いたら **Tab キー** を押してから **Enter**（Authorizeボタンが有効になる）
...
### キャッシュが古い場合
`sw.js` の `CACHE = 'travel-plan-vX'` の番号を1つ上げてpushする。
```bash
cp "C:/Users/SG030/OneDrive/Desktop/ToDoトラベル/travel-app/sw.js" /tmp/travel-app-upload/sw.js
cd /tmp/travel-app-upload
git add sw.js
